  ...
  methods:{
    ...
    Register(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post("https://vernelproxy.dynv6.net/proxy/frp-hat.top:49728/register",this.ruleForm).then(e => {
            if (e.data.code === 210){
    ...
    async checkNameFrom() {
      try {
        const response = await axios.get(`https://vernelproxy.dynv6.net/proxy/frp-hat.top:49728/register/checkName/${this.ruleForm.username}`);
